...
export default function HomePage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus(`עודכן: ${parsed.action === 'add' ? 'הוספנו' : 'הורדנו'} פריטים`);
    } catch (error) {
      console.error('Submit error:', error);
      setStatus('משהו השתבש בעיבוד ה-AI');
    }
      ...
      <section className="max-w-xl mx-auto">
        ...
        <form onSubmit={onSubmit} className="mb-8 bg-white p-6 rounded-2xl shadow-md">
          ...
          <input
            ...
            placeholder='לדוגמה: "קנינו 3 חלב וגם קילו עגבניות"'
          ...
          <button
            type="submit"
            className="mt-4 w-full rounded-xl bg-blue-600 px-4 py-4 text-white text-xl font-bold hover:bg-blue-700 shadow-lg"
          >
            שלח ל-AI
          </button>
        </form>
        ...
        <div className="flex justify-between items-center mb-4 px-2">
          <h2 className="text-xl font-semibold">רשימת מלאי</h2>

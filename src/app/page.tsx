    ...
    <main className="min-h-screen bg-slate-50 font-sans pb-20 text-slate-900" dir="rtl">
      <header className={`bg-gradient-to-r ${headerGradient} text-white p-4 shadow-xl sticky top-0 z-[1000] transition-colors duration-500`}>
        <div className="max-w-2xl mx-auto flex justify-between items-center">
          <h1 className="text-2xl font-black cursor-pointer drop-shadow-md pointer-events-auto" onClick={() => {setActiveView('HOME'); setSearchTerm('');}}>הבית של ניאו 🏠</h1>
          <nav className="flex gap-1 items-center">
            <button onClick={handleRefresh} className="p-2 rounded-xl bg-black/10 hover:bg-black/20 pointer-events-auto">🔄</button>
            <button onClick={() => {setActiveView('INVENTORY'); setStatus('');}} className={`px-3 py-2 rounded-xl font-bold text-xs transition-all pointer-events-auto ${activeView === 'INVENTORY' ? 'bg-white text-teal-700 shadow-lg' : 'bg-black/10'}`}>מלאי</button>
            <button onClick={() => {setActiveView('SHOPPING'); setStatus('');}} className={`px-4 py-2 rounded-xl font-bold text-sm transition-all pointer-events-auto ${activeView === 'SHOPPING' ? 'bg-white text-rose-700 shadow-lg' : 'bg-black/10'}`}>קניות</button>
            <button onClick={() => {setActiveView('TASKS'); setStatus('');}} className={`px-3 py-2 rounded-xl font-bold text-xs transition-all pointer-events-auto ${activeView === 'TASKS' ? 'bg-white text-purple-700 shadow-lg' : 'bg-black/10'}`}>משימות</button>
          </nav>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto p-4 relative z-10">
        ...
        {activeView === 'HOME' && (
          <div className="grid grid-cols-1 gap-4 mt-6">
            <button onClick={() => setActiveView('INVENTORY')} className="p-6 bg-white rounded-[2rem] shadow-lg border-b-8 border-teal-500 flex items-center justify-between active:scale-95 transition-all pointer-events-auto">
            ...
            <button onClick={() => setActiveView('SHOPPING')} className="p-6 bg-white rounded-[2rem] shadow-lg border-b-8 border-rose-500 flex items-center justify-between active:scale-95 transition-all pointer-events-auto">
            ...
            <button onClick={() => setActiveView('TASKS')} className="p-6 bg-white rounded-[2rem] shadow-lg border-b-8 border-purple-500 flex items-center justify-between active:scale-95 transition-all pointer-events-auto">
        ...
        {/* INVENTORY & SHOPPING VIEWS */}
        {(activeView === 'INVENTORY' || activeView === 'SHOPPING') && (
           <>
            <div className="mb-4 space-y-3">
              <input type="text" placeholder="🔍 חפש מוצר..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full p-4 rounded-2xl shadow-sm border-none bg-white focus:ring-2 focus:ring-amber-400 pointer-events-auto" />
              <div className="flex bg-white rounded-xl shadow-sm p-1">
                <button onClick={() => setSortBy('category')} className={`flex-1 py-2 rounded-lg text-xs font-bold pointer-events-auto ${sortBy === 'category' ? 'bg-amber-100 text-amber-900' : 'text-slate-400'}`}>📁 קטגוריות</button>
                <button onClick={() => setSortBy('name')} className={`flex-1 py-2 rounded-lg text-xs font-bold pointer-events-auto ${sortBy === 'name' ? 'bg-indigo-100 text-indigo-900' : 'text-slate-400'}`}>🔤 א-ת</button>
              </div>
            </div>
            <div className="bg-white p-5 rounded-[2rem] shadow-xl mb-6 border border-slate-50 relative z-20">
              <form onSubmit={handleUpdate} className="space-y-3">
                <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="עדכון חופשי (למשל: 3 חלב, פסטה)" className="w-full p-4 bg-slate-50 rounded-2xl text-sm min-h-[70px] border-none focus:ring-2 focus:ring-amber-300 pointer-events-auto" />
                <button type="submit" className={`w-full p-4 rounded-2xl text-white font-black text-lg active:scale-95 transition-all shadow-md pointer-events-auto ${activeView === 'INVENTORY' ? 'bg-teal-600 hover:bg-teal-700' : 'bg-rose-600 hover:bg-rose-700'}`}>עדכן ✨</button>
              </form>
              {status && (
      ...
      <div className="text-right flex-1">
        {editingId === item.id ? (
          <div className="flex gap-2">
            <input autoFocus value={editNameValue} onChange={e => setEditNameValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveEditedName(item.id, 'inventory_items')} className="bg-teal-50 px-3 py-1 rounded-xl w-[140px] font-bold border-b-2 border-teal-500 outline-none pointer-events-auto" />
            <button onClick={() => saveEditedName(item.id, 'inventory_items')} className="bg-green-100 text-green-700 px-2 rounded-lg text-sm pointer-events-auto">✅</button>
          </div>
        ) : (
          <span onClick={() => {setEditingId(item.id); setEditNameValue(item.item_name);}} className="block font-bold text-lg text-slate-800 cursor-pointer hover:text-teal-600 transition-colors pointer-events-auto">{item.item_name}</span>
        )}
        <p className="text-[9px] uppercase font-bold text-slate-400 mt-1">{item.category} • {item.location}</p>
      </div>
      <div className="flex items-center gap-2" dir="ltr">
        <button onClick={onHalf} className="w-8 h-8 rounded-full bg-amber-100 text-amber-600 text-xs font-black shadow-sm hover:bg-amber-200 transition-colors pointer-events-auto">½</button>
        <button onClick={onMinus} className="w-9 h-9 rounded-full bg-rose-100 text-rose-600 font-black shadow-sm hover:bg-rose-200 transition-colors pointer-events-auto">-</button>
        <span className={`min-w-[36px] text-center text-xl font-black ${item.quantity <= 2 ? 'text-rose-600' : 'text-slate-700'}`}>{item.quantity}</span>
        <button onClick={onPlus} className="w-9 h-9 rounded-full bg-emerald-100 text-emerald-600 font-black shadow-sm hover:bg-emerald-200 transition-colors pointer-events-auto">+</button>
      </div>
    </div>
  );
}
...
function ShoppingCard({ item, editingId, editNameValue, setEditingId, setEditNameValue, saveEditedName, onRemove }: any) {
  return (
    <div className="flex justify-between items-center gap-4 bg-white p-5 rounded-[2rem] shadow-sm border border-rose-50 hover:border-rose-100 transition-colors">
      <div className="flex items-center gap-4 flex-1">
        <button onClick={() => onRemove(item)} className="bg-rose-100 text-rose-600 hover:bg-rose-200 px-4 py-2 rounded-2xl text-xs font-black shadow-sm transition-colors pointer-events-auto">הסר</button>
        {editingId === item.id ? (
          <input autoFocus value={editNameValue} onChange={e => setEditNameValue(e.target.value)} onKeyDown={e => e.key === 'Enter' && saveEditedName(item.id, 'shopping_list')} className="bg-rose-50 px-3 py-1 rounded-xl w-[140px] font-bold border-b-2 border-rose-500 outline-none pointer-events-auto" />
        ) : (
          <span onClick={() => {setEditingId(item.id); setEditNameValue(item.item_name);}} className="font-bold text-lg text-slate-800 cursor-pointer hover:text-rose-600 transition-colors pointer-events-auto">{item.item_name}</span>
        )}
      </div>
    </div>
  );

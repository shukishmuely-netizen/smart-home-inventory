...
export default function HomePage() {
  ...
  // ----- הפונקציה המעודכנת: שמירת משימה והקפצה לוואטסאפ -----
  const handleAddTask = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTask.title) return;
    ...
    if (!error) {
      ...
      // 2. קידוד ההודעה לפורמט של קישור והקפצה לוואטסאפ
      const encodedMessage = encodeURIComponent(waMessage);
      window.open(`https://wa.me/?text=${encodedMessage}`, '_blank');

      // 3. איפוס הטופס ועדכון האפליקציה
      setNewTask({ title: '', description: '', urgency: 'סטנדרטית', assignee: '👫 כולם', target_date: today, status: 'לא התחלתי' });
          ...
          <div className="space-y-6 mt-4">
            ...
            {showTaskForm && (
              <form onSubmit={handleAddTask} className="bg-white p-6 rounded-[2rem] shadow-xl border border-purple-100 space-y-4 animate-in fade-in slide-in-from-top-4 relative z-20">
                ...
                <button type="submit" className="w-full bg-emerald-500 text-white p-4 rounded-xl font-black shadow-md active:scale-95 transition-all pointer-events-auto">שמור משימה ושלח לוואטסאפ 💬</button>
              </form>
            )}
                  ...
                  <div key={task.id} className={`p-5 rounded-[2rem] border-r-8 shadow-sm transition-all ${urgencyColor}`}>
                    ...
                    <div className="flex bg-slate-200 p-1 rounded-xl mt-2">
                      <button 
                        ...
                        className={`flex-1 py-2 rounded-lg text-[11px] font-black transition-all pointer-events-auto ${task.status === 'לא התחלתי' || !task.status ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                      >
                        {isPlural ? 'לא התחלנו' : 'לא התחלתי'}
                      </button>
                      <button 
                        onClick={() => updateTaskStatus(task.id!, 'בתהליך')} 
                        className={`flex-1 py-2 rounded-lg text-[11px] font-black transition-all pointer-events-auto ${task.status === 'בתהליך' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                      >
                        בתהליך
                      </button>
                      <button 
                        onClick={() => updateTaskStatus(task.id!, 'סיימתי')} 
                        className={`flex-1 py-2 rounded-lg text-[11px] font-black transition-all pointer-events-auto text-slate-500 hover:text-emerald-600`}
                      >
      ...
      <div className="mt-4 pt-4 border-t border-slate-200">
        ...
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 p-2 rounded-xl text-xs font-bold bg-white border border-slate-200 pointer-events-auto" 
          />
          <button 
            onClick={() => onRestore(task.id!, newDate)} 
            className="bg-slate-300 text-slate-700 hover:bg-slate-400 px-4 py-2 rounded-xl text-xs font-black shadow-sm transition-colors pointer-events-auto"
          >
            שחזר משימה ↺
          </button>
        </div>

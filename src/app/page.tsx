...
export default function HomePage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'category'>('name'); // ברירת מחדל: לפי שם (א-ת)
  const [isMenuOpen, setIsMenuOpen] = useState(false); // מצב התפריט הנפתח
  ...
  // Optimistic UI Update - עדכון מיידי של הממשק לפני השרת כדי למנוע איטיות
  const updateExactQuantity = (item: Item, newQty: number) => {
    // 1. עדכון מיידי של המסך
  ...
  const saveItemAI = async (item: Item, action: 'add' | 'remove' = 'add') => {
    ...
    if (existing) {
      ...
      const newItem = { item_name: name, quantity: item.quantity, category: item.category || 'כללי', location: item.location || 'מזווה', household_id: '92e1a987-99b7-41ec-93fb-ae2ada2bcf72' };
  ...
  const handleRemoveFromShopping = async (shopItem: any) => {
    ...
    setShoppingList(prev => prev.filter(i => i.id !== shopItem.id)); // Optimistic
  ...
  const handleRestoreToShopping = async (shopItem: any) => {
    ...
    setShoppingList(prev => [...prev, shopItem]); // Optimistic
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans pb-20 text-slate-900" dir="rtl">
      ...
      {/* תפריט עליון ושורת חיפוש מקובעת */}
      <header className={`bg-gradient-to-r ${headerGradient} text-white shadow-xl sticky top-0 z-[1000] transition-colors duration-500`}>
        <div className="max-w-2xl mx-auto p-4 flex flex-col gap-3">
          ...
          <div className="flex justify-between items-center relative">
            <h1 className="text-2xl font-black cursor-pointer drop-shadow-md" onClick={() => {setActiveView('HOME'); setSearchTerm('');}}>הבית של ניאו 🏠</h1>
            
            <div className="flex items-center gap-2">
              <button onClick={handleRefresh} className="p-2 rounded-xl bg-black/10 hover:bg-black/20 font-black" title="רענן עמוד">🔄</button>
              
              {/* תפריט המבורגר במקום כפתורים בשורה */}
              <div className="relative">
                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="px-4 py-2 rounded-xl bg-black/20 hover:bg-black/30 font-bold tracking-wide shadow-inner flex items-center gap-2">
                  <span>☰</span> תפריט
                </button>
                ...
                {isMenuOpen && (
                  <div className="absolute left-0 top-12 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col text-slate-800 z-[1001] animate-in fade-in slide-in-from-top-2">
                    <button className="p-4 text-right font-black hover:bg-teal-50 border-b flex justify-between" onClick={() => {setActiveView('INVENTORY'); setIsMenuOpen(false);}}><span>מלאי</span><span>📦</span></button>
                    <button className="p-4 text-right font-black hover:bg-rose-50 border-b flex justify-between" onClick={() => {setActiveView('SHOPPING'); setIsMenuOpen(false);}}><span>קניות</span><span>🛒</span></button>
                    <button className="p-4 text-right font-black hover:bg-purple-50 flex justify-between" onClick={() => {setActiveView('TASKS'); setIsMenuOpen(false);}}><span>משימות</span><span>📌</span></button>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* שורת חיפוש צמודה לתפריט (מוסתרת רק במסך הבית) */}
          {activeView !== 'HOME' && (
            <div className="relative w-full">
              <input 
                type="text" 
                placeholder={`🔍 חפש ${activeView === 'INVENTORY' ? 'במלאי' : activeView === 'SHOPPING' ? 'בקניות' : 'במשימות'}...`} 
                value={searchTerm} 
      ...
      {/* רקע שחור שקוף לסגירת התפריט בלחיצה בחוץ */}
      ...
      <div className="max-w-2xl mx-auto p-4 relative z-10">
        ...
        {/* פאנל סידור בלבד (החיפוש עבר למעלה) */}
        ...
        {/* טופס AI למלאי וקניות */}
        {(activeView === 'INVENTORY' || activeView === 'SHOPPING') && (
          <div className="bg-white p-5 rounded-[2rem] shadow-xl mb-6 border border-slate-50 relative z-20">
            <form onSubmit={handleUpdate} className="space-y-3">
              <textarea value={input} onChange={e => setInput(e.target.value)} placeholder={activeView === 'INVENTORY' ? "מה הוספנו למלאי? (למשל: 2 חלב, חצי אבטיח)" : "מה חסר? (למשל: עגבניות, 3 קולה)"} className="w-full p-4 bg-slate-50 rounded-2xl text-sm min-h-[70px] border-none focus:ring-2 focus:ring-amber-300 pointer-events-auto" />
        ...
        {/* שאלות סיווג למלאי (כולל עדכון UI אופטימי) */}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {options.map(opt => (
                        <button key={opt} onClick={async () => {
                          ...
                          setPendingItems(prev => prev.filter(p => p.item_name !== item.item_name)); // Optimistic remove box
...
// כרטיס מלאי עם תפריט נפתח לכמות
  ...
  return (
    <div className="flex justify-between items-center bg-white p-4 rounded-[1.5rem] shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-2" dir="ltr">
        ...
        {/* תפריט כמויות לחיץ (Select) חבוי מעל המספר */}

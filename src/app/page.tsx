...
export default function HomePage() {
  ...
  const saveItemAI = async (item: Item) => {
    ...
    if (existing) {
      ...
      await supabase.from('inventory_items').insert([{ item_name: name, quantity: Math.max(0, item.quantity || 1), category: item.category || 'כללי', location: item.location || 'מזווה', household_id: '92e1a987-99b7-41ec-93fb-ae2ada2bcf72' }]);
    }
    fetchData();
  };
